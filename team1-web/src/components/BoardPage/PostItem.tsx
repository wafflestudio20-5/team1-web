...
export default function PostItem({ Post, boardId }: { Post: Post; boardId: number }) {
  return (
    <Link to={`/${boardId}/v/${Post.postId}`} className={styles['Post']}>
      <h2 className={styles['medium']}>{Post.title ? Post.title : '(제목없음)'}</h2>
      <p className={styles['small']}>{Post.contents}</p>
      <time className={styles['small']}>
        {Post?.createdAt === undefined ? '' : formattedTime(Post?.createdAt)}
      </time>
      <h3 className={styles['small']}>{Post?.isWriterAnonymous ? '익명' : Post?.nickname}</h3>
